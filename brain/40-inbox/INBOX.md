...
- [2026-03-08T20:08:51Z] Consolidated hard workflow rules (2026-03-08): (1) '后台执行' strictly means WakeBridge via tools/runlong.sh, not plain background exec; (2) runlong requires TaskBoard up, and wakebridge should fail fast if TaskBoard(:9876) is down; (3) never emit HEARTBEAT_OK for WB_START/WB_DONE/Exec status messages; treat them as task status events; (4) TPU remote is multi-host by default, must use --worker=all for init/ops; (5) TPU init troubleshooting order: check occupancy via sudo lsof /dev/accel* or /dev/vfio/*, if occupied by others delete mapping (do not kill without explicit authorization), if unoccupied clear /tmp/*tpu* then retry; (6) default local-dev python is /kmh-nfs-ssd-us-mount/code/hanhong/miniforge3/bin/python and do not casually install into shared env. (P0) -> entries/2026-03-08-20260308T200851Z.md
- [2026-03-08T20:10:35Z] Created dedicated note: brain/10-domains/background-execution-playbook.md. Trigger phrase: '温习你后台执行的那个笔记'. Enforces WakeBridge-only background execution, TaskBoard precheck, WB status handling, and heartbeat guardrails. (P0) -> entries/2026-03-08-20260308T201035Z.md
- [2026-03-08T20:54:17Z] 整理 TPU init 实战排障流程到 brain/10-domains/tpu-init-troubleshooting-playbook.md：包含 warning/错误含义、对应动作、后台执行模板、以及 delete+reapply 成功收敛经验。 (P1) -> entries/2026-03-08-20260308T205417Z.md
- [2026-03-08T21:05:48Z] Tooling preference update: do NOT use sessions_spawn/subagents for coding work. Use TaskBoard/WakeBridge-based background task scheduling system only. (P0) -> entries/2026-03-08-20260308T210548Z.md
- [2026-03-08T22:31:07Z] Memory policy update: replaced 'always manually set an alarm' with auto system-alarm model. For non-alarm tasks, prefer --timeout-sec > 20 so WakeBridge creates system-alarm(parent:<task_id>) and auto-cancels on early completion; if missing, emit immediate WB_WARN at start. Manual alarm(tag=alarm) is optional for ad-hoc checks. (P0) -> entries/2026-03-08-20260308T223107Z.md
